	...
	<script>
		...
		function get_phoneting_data(){
		    ...
		    for(var i=0;i<obj.length;i++){
		      var line="<tr>";
					line+="<td><form name='upload-form' id=img_form"+obj[i].no+" action='upload3.php' method='post' enctype='multipart/form-data'><input style='display:none' type=file name='upload' id=img"+obj[i].no+"><input type='hidden' name='no' value='"+obj[i].no+"'/></form><img width=150 height=150 onclick='document.getElementById(\"img"+obj[i].no+"\").click();' src='"+obj[i].uri+"'></td>";
		...
		function update_content(no){
			...
			$.post("../query/update_contents.php",{no:no,title:title,text:text,phone:phone}).done(function(data){
				alert("성공적으로 수정되었습니다.");
				if(document.getElementById('img'+no).value!=""){
					document.getElementById('img_form'+no).submit();
				}else{
					location.reload();
				}
				//location.reload();
